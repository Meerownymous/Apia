...
public sealed class UserFeedProjectionTests
{
    ...
    public async Task BuildsFeed_ReturnsOneItem()
    {
        ...
        Assert.Single(await memory.Aggregate(new UserFeedQuery(user1.UserId, Limit: 20)).ToListAsync());
    }
    ...
    public async Task BuildsFeed_ReturnsCorrectPostId()
    {
        ...
        Assert.Equal(post.PostId, (await memory.Aggregate(new UserFeedQuery(user1.UserId, Limit: 20)).ToListAsync())[0].PostId);
    }
    ...
    public async Task BuildsFeed_ReturnsCorrectAuthorName()
    {
        ...
        Assert.Equal(user1.Username, (await memory.Aggregate(new UserFeedQuery(user1.UserId, Limit: 20)).ToListAsync())[0].AuthorName);
    }
    ...
    public async Task BuildsFeed_ReturnsCorrectCommentCount()
    {
        ...
        Assert.Equal(1, (await memory.Aggregate(new UserFeedQuery(user1.UserId, Limit: 20)).ToListAsync())[0].CommentCount);
    }
    ...
    public async Task ReturnsEmpty_WhenUserHasNoPosts()
    {
        ...
        Assert.Empty(await memory.Aggregate(new UserFeedQuery(user.UserId, Limit: 20)).ToListAsync());
    }
    ...
    public async Task ReturnsEmpty_WhenUserNotFound()
    {
        ...
        Assert.Empty(await memory.Aggregate(new UserFeedQuery(Guid.NewGuid(), Limit: 20)).ToListAsync());
    }
}
