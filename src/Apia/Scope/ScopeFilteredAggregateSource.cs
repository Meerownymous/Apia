...
public sealed class ScopeFilteredAggregateSource<TRecord, TFilter>(
    IAggregateSource<TRecord> inner,
    IScope<TRecord, TFilter> scope,
    TFilter filter) : IAggregateSource<TRecord>
{
    public IAsyncEnumerable<TRecord> From<TQuery>(TQuery query)
        => scope.AsLinq(filter).Match(
            linq => query is AllOf<TRecord>
                ? inner.From(new LinqQuery<TRecord>(linq))
                : inner.From(query).Where(r => scope.Includes(r, filter)),
            _ => inner.From(query).Where(r => scope.Includes(r, filter))
        );
}
