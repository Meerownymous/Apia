...
/// Usage: new PostgresMutableCatalog&lt;PostRecord&gt;(p => p.PostId)
/// </summary>
public sealed class PostgresMutableCatalog<TResult>(Func<TResult, Guid> idOf) : IMutableCatalog<TResult>
    where TResult : notnull
{
    internal BoundPostgresMutableCatalog<TResult> Bind(IDocumentSession session)
        => new(session, idOf);

    public Func<TResult, Guid> IdOf
        => throw new InvalidOperationException("Use IMemory.Catalog<T>() to access this store.");
...
internal sealed class BoundPostgresMutableCatalog<TResult> : IMutableCatalog<TResult> where TResult : notnull
{
    private readonly IDocumentSession session;
    private readonly Func<TResult, Guid> idOf;
    private readonly ConcurrentDictionary<Guid, uint> loadedVersions = new();

    internal BoundPostgresMutableCatalog(IDocumentSession session, Func<TResult, Guid> idOf)
    {
